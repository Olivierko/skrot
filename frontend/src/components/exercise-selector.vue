    ...
    <div class="box p-0 is-radiusless">
      <nav class="panel is-dark">
        <div class="panel-heading is-radiusless">
          Select exercise(s)
          <button @click.prevent="isVisible = false" class="delete is-medium is-pulled-right"></button>
        </div>
        <div class="panel-block">
          <p class="control has-icons-left">
            <input v-model="query" class="input" type="search" pattern="([^\s][A-z0-9À-ž\s]+)" @keyup="stopPropagation" placeholder="Search" tabindex="0" />
        ...
        <div class="exercise-container p-1">
          ...
          >
            ...
            <div class="tags is-pulled-right">
              <span class="tag">{{ findMuscleGroup(exercise.muscleGroupId)?.name ?? "UNKNOWN_MUSCLE_GROUP" }}</span>
        ...
        <div class="container">
          <div class="tabs is-toggle is-fullwidth">
            <ul>
              ...
              <li>
                <button @click.prevent="confirm" class="button is-success is-fullwidth is-radiusless" :disabled="!selections.length">
                  Add {{ `(${selections.length})` }}
                </button>
              </li>
            </ul>
          </div>
